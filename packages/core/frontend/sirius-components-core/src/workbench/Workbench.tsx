...
export const Workbench = forwardRef<WorkbenchConfigurationSupplier, WorkbenchProps>(
  (
    {
      editingContextId,
    ...
    ref: ForwardedRef<WorkbenchConfigurationSupplier>
  ) => {
    const { classes } = useWorkbenchStyles();

    const initialRepresentations: RepresentationMetadata[] =
      initialWorkbenchConfiguration?.mainPanel?.representationEditors.map(
        (configuration) => configuration.representationMetadata
      ) ?? (initialRepresentationSelected ? [initialRepresentationSelected] : []);
    ...
    const refWorkbenchSidePanelConfigurationsSupplier = useRef<WorkbenchSidePanelConfigurationsSupplier>({
      getSidePanelConfigurations() {
        return null;
      },
    });
    const refWorkbenchMainPanelConfigurationSupplier = useRef<WorkbenchMainPanelConfigurationSupplier>({
      ...
      return {
        getConfiguration: () => {
          return {
            ...
            mainPanel: refWorkbenchMainPanelConfigurationSupplier.current.getMainPanelConfiguration(),
          };
        },
      };
    });
    ...
    const leftPanelConfiguration: WorkbenchSidePanelConfiguration | null =
      initialWorkbenchConfiguration?.sidePanels?.find((configuration) => configuration.id === 'left') ?? null;
    const rightPanelConfiguration: WorkbenchSidePanelConfiguration | null =
      initialWorkbenchConfiguration?.sidePanels?.find((configuration) => configuration.id === 'right') ?? null;
